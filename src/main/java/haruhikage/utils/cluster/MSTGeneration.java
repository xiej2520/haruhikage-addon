...
public class MSTGeneration {
    ...
    public static Set<ChunkPos> generateStructureMap(ChunkPos clusterCornerPos, List<MSTGeneration.MSTEdge> MSTEdgeList, List<ChunkPos> clusterChunks) {
        ...
        if (clusterChunks.isEmpty() || MSTEdgeList.isEmpty()) return structureMap;
    ...
    public static void buildLoadingGrid(ChunkPos clusterCornerPos, List<MSTGeneration.MSTEdge> MSTEdgeList, List<ChunkPos> clusterChunks, World world, int y, IBlockState state) {
        Set<ChunkPos> visited = new HashSet<>();
        Set<BlockPos> blockVisited = new HashSet<>();
        Map<ChunkPos, List<ChunkPos>> graph = new HashMap<>();
        if (clusterChunks.isEmpty() || MSTEdgeList.isEmpty()) return;
    ...
    public static void buildLoadingGridDFS(ChunkPos startChunk, Map<ChunkPos, List<ChunkPos>> graph, Set<ChunkPos> visited, Set<BlockPos> blockVisited,
                                           World world, int y, IBlockState buildingBlock) {
        for (ChunkPos endChunk : graph.getOrDefault(startChunk, Collections.emptyList())) {
    ...
    public static void connectBlocksAsPath(World world, int y, IBlockState buildingBlock, ChunkPos c1, ChunkPos c2, Set<BlockPos> blockVisited) {
        ...
        while (true) {
            ...
            if (!blockVisited.contains(pos)) {
                if (currentX >> 4 != (currentX + stepX) >> 4)
                    ...
                    world.setBlockState(pos, Blocks.CHEST.getDefaultState(), 2);
                else world.setBlockState(pos, buildingBlock, 2);
    ...
    public static void setChunkCentre(World world, int y, ChunkPos chunk) {
        ...
        world.setBlockState(new BlockPos(centreX, y, centreZ), Blocks.HOPPER.getDefaultState().withProperty(BlockHopper.FACING, EnumFacing.DOWN)
            .withProperty(BlockHopper.ENABLED, true), 2);
        world.setBlockState(new BlockPos(centreX, y, centreZ).up(), Blocks.TORCH.getDefaultState().withProperty(BlockTorch.FACING, EnumFacing.UP));
        TileEntity tileEntity = world.getTileEntity(new BlockPos(centreX, y, centreZ));
